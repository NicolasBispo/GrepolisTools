...
class GrepolisPuppeteer {
    ...
    async executarLogin(usuario, senha) {
        ...
        try {
            ...
            console.log(this.yellow + '[GREPOLIS-PUPPETEER]' + this.red + '-' + this.white + 'Erro no login' + this.reset);
            return false;
        } catch (error) {
            // Se ocorrer uma exceção, significa que o seletor não foi encontrado
            // e o login foi bem-sucedido
            console.log(this.yellow + '[GREPOLIS-PUPPETEER]' + this.red + '-' + this.white + 'Login completo no site' + this.reset);
    ...
    async selecionarMundo(mundo) {
        ...
        if (botaoMundo) {
            ...
            await page.waitForNavigation();
            console.log(this.yellow + '[GREPOLIS-PUPPETEER]' + this.red + '-' + this.white + 'Navegação completa' + this.reset);
